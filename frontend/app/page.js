...
const VK_AUTH_REDIRECT = "https://vk-inbox.vercel.app/auth";
...
export default function Home() {
  ...
  const loadVkGroups = async (token) => {
    ...
    try {
      ...
      console.error(error);
      setAuthError(
        "Не удалось загрузить группы VK. Если VK заблокирует браузерный запрос, понадобится маленький backend-прокси."
      );
      setVkGroups([]);
    }
  };
